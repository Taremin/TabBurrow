...
// storage のモック
const { mockFindTabByCanonicalUrl, mockUpdateTab } = vi.hoisted(() => ({
  mockFindTabByCanonicalUrl: vi.fn(),
  mockUpdateTab: vi.fn(),
}));

vi.mock('../storage/index.js', () => ({
  findTabByCanonicalUrl: mockFindTabByCanonicalUrl,
...
describe('tabEvents', () => {
  ...
  describe('Tab Active/Time Tracking', () => {
  ...
  describe('checkAndUpdateLastAccessed', () => {
    it('キャッシュに含まれるURLにアクセスした際、DBのlastAccessedが更新され、tabs-changedが送信されること', async () => {
      const url = 'https://example.com/target';
      savedTabUrlsCache.add(url);
      
      const mockTab = { id: 'tab-123', url, canonicalUrl: url, lastAccessed: 1000 };
      mockFindTabByCanonicalUrl.mockResolvedValue(mockTab);

      const { checkAndUpdateLastAccessed } = await import('./tabEvents');
      await checkAndUpdateLastAccessed(url);

      expect(mockFindTabByCanonicalUrl).toHaveBeenCalledWith(url);
      expect(mockUpdateTab).toHaveBeenCalledWith('tab-123', expect.objectContaining({
        lastAccessed: expect.any(Number)
      }));
      expect(browser.runtime.sendMessage).toHaveBeenCalledWith({ type: 'tabs-changed' });
    });

    it('キャッシュに含まれないURLにアクセスした際、DBの更新は行われないこと', async () => {
      const url = 'https://example.com/target';

      const { checkAndUpdateLastAccessed } = await import('./tabEvents');
      await checkAndUpdateLastAccessed(url);

      expect(mockFindTabByCanonicalUrl).not.toHaveBeenCalled();
      expect(mockUpdateTab).not.toHaveBeenCalled();
    });

    it('5秒以内の連続したアクセスでは、DBの更新が2回目以降スキップされること', async () => {
      const url = 'https://example.com/target';
      savedTabUrlsCache.add(url);
      
      // 直近の最終アクセス時間を現在時刻（に近い時間）に設定
      const mockTab = { id: 'tab-123', url, canonicalUrl: url, lastAccessed: Date.now() - 1000 }; // 1秒前
      mockFindTabByCanonicalUrl.mockResolvedValue(mockTab);
